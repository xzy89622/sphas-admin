<template>
  <el-card class="page-card">
    <template #header>
      <div class="card-header">
        <div class="title-wrap">
          <div class="title">运动方案管理</div>
          <div class="desc">维护个性化运动推荐方案，支持筛选、新增、编辑、删除</div>
        </div>
    ...
    <div class="filter-bar">
      ...
      <el-select v-model="query.intensity" placeholder="强度" clearable style="width: 140px">
        <el-option label="低强度" value="LOW" />
        <el-option label="中强度" value="MID" />
    ...
    <el-table :data="list" border v-loading="tableLoading">
      ...
      <el-table-column prop="intensity" label="强度" width="120">
        <template #default="{ row }">
          <el-tag>{{ intensityText(row.intensity) }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-form :model="form" label-width="90px">
      ...
      <el-form-item label="方案内容" required>
        <el-input
          ...
          type="textarea"
          :rows="6"
          placeholder="请输入运动方案内容"
        />
      </el-form-item>

      <el-form-item label="运动强度">
        <el-select v-model="form.intensity" style="width: 180px">
          <el-option label="低强度" value="LOW" />
          <el-option label="中强度" value="MID" />
          <el-option label="高强度" value="HIGH" />
        </el-select>
...
import { sportPlanAdminPage, sportPlanSave, sportPlanDelete } from "../api/sportPlan";
...
async function fetchList() {
  ...
  try {
    const pageData = await sportPlanAdminPage({
      pageNum: page.pageNum,
      pageSize: page.pageSize,
      name: query.name || undefined,
      bmiLevel: query.bmiLevel || undefined,
      intensity: query.intensity || undefined,
...
async function save() {
  ...
  try {
    await sportPlanSave({
      ...
      intensity: form.intensity ? String(form.intensity).trim() : "",
      status: form.status,
    });
...
async function removeRow(row) {
  await ElMessageBox.confirm(`确认删除运动方案【${row.name}】吗？`, "提示", {
    type: "warning",
  });

  await sportPlanDelete(row.id);
